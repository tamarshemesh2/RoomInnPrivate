...
class RoomVectorFactory() {
    ...
    fun getVectorDrawable(
        ...
        val binXml =
            createBinaryDrawableXml(width, height, viewportWidth, viewportHeight, paths)
        try {
            ...
            @SuppressLint("PrivateApi") val xmlBlock =
                Class.forName("android.content.res.XmlBlock")
            val xmlBlockConstr: Constructor<out Any> =
                xmlBlock.getConstructor(ByteArray::class.java)
            ...
            Log.e(RoomVectorFactory::class.java.simpleName, "Vector creation failed", e)
        }
        return null
    }

    private fun createBinaryDrawableXml( width: Int, height: Int,
        viewportWidth: Float, viewportHeight: Float,
        paths: List<PathData>): ByteArray {
        val stringPool: ArrayList<Array<ByteArray>> = ArrayList(listOf(BIN_XML_STRINGS))
        for (path in paths) {
            stringPool.add(arrayOf(path.data))
        }
        val bb: ByteBuffer = ByteBuffer.allocate(8192) // Capacity might have to be greater.
        bb.order(ByteOrder.LITTLE_ENDIAN)
        var posBefore: Int

        // ==== XML chunk ====
        // https://justanapplication.wordpress.com/2011/09/22/android-internals-binary-xml-part-two-the-xml-chunk/
        bb.putShort(Companion.CHUNK_TYPE_XML) // Type
